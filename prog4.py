...
def entropy(data):
    ...
    en=0
    if(p_ex>0):
        en=-(p_ex/float(total_ex))*(math.log(p-ex,2)-math.log(total_ex,2))
    if(n_ex>0):
...
def gain(en_s,data_s,attrib):
    ...
    gain=en_s
    for value in values:
        gain-= len(data_s.loc[data_s[attrib]==values])/float(len(data_s))*entropy(data_s.loc[data_s[attrib]==value])
        return gain
def get_attr(data):
    en_s=entropy(data)
    attribute=""
...
def get_rules(root,rule,rules):
    if not root.branch:
        rules.append(rules[:-1]+"=>"+root.label)
        return rules
    for val in root.branch:
        #print(val)
        get_rules(root.branch[val],rule.label+"="+str(val)+"^",rules)
        return rules
